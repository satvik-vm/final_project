...
def train(base_loader, base_loader_test, cnn_model, dnn_model, bridge_model, stop_epoch, dnn_optimizer, dnn_scheduler, bridge_optimizer, bridge_scheduler, params):
	...
	bridge_initial_weights = {name: param.clone() for name, param in bridge_model.named_parameters()}
	dnn_initial_weights = {name: param.clone() for name, param in dnn_model.named_parameters()}
	...
	for epoch in range(stop_epoch):
		...
		for batch_idx, (input_var, target_var) in enumerate(base_loader):
			...
			if batch_idx%50 == 0 or batch_idx == stop_epoch - 1:
				# print(len(correct))
				print('{0}/{1}'.format(batch_idx,len(base_loader)), "Mixup Layer:", mixup_layer, end=" | ")
				print('Loss: %.3f | Acc: %.3f%%  '
							 % (tot_bloss * 100/(batch_idx+1),100.*correct/total ))

		# for name, param in bridge_model.named_parameters():
		# 	# print(name)
		# 	if not torch.equal(bridge_initial_weights[name], param):
		# 		print(f"Weights for bridge model layer {name} have been updated.")
		# 	else:
		# 		# print(f"Weights for {name} did not change.")
		# 		Exception(f"Weights for {name} did not change.")

		# for name, param in dnn_model.named_parameters():
		# 	if not torch.equal(dnn_initial_weights[name], param):
		# 		print(f"Weights for dnn model layer {name} have been updated.")
		# 	else:
		# 		# print(f"Weights for {name} did not change.")
		# 		Exception(f"Weights for {name} did not change.")
		...
		print('Loss: %.3f | Acc: %.3f%%'
							 % (eval_loss/(batch_idx+1), 100.*eval_accuracy/total ))
...
if __name__ == "__main__":
	...
	cnn_model = wrn_mixup_model.wrn28_10(num_classes=params.num_classes).to(device)

	checkpoint = torch.load(cnn_model_path, weights_only=False)

	cnn_model.load_state_dict(checkpoint['state'])

	dnn_model = fully_connected.mixup_model(num_classes=params.num_classes, inputsize=bridge_output_size).to(device)

	bridge_sizes = [bridge_input_size, 300, bridge_output_size]
	...
	bridge_optimizer = optim.Adam(bridge_model.parameters(), lr=0.01)
