...
class WideResNet(nn.Module):
	...
	def forward(self, x, target= None, mixup=False, mixup_hidden=True, mixup_alpha=None , lam = 0.4):
		if target is not None:
			...
			# print("Size 3:\t",out.size())
			out = self.block2(out)

			if layer_mix == 2:
				out, target_a , target_b , lam = mixup_data(out, target, lam=lam)

			# print("Size 4:\t",out.size())
			out = self.block3(out)
			if  layer_mix == 3:
				out, target_a , target_b , lam = mixup_data(out, target, lam=lam)

			# print("Size 5:\t",out.size())

			out = self.relu(self.bn1(out))
			# print("Size 6:\t",out.size())
			out = F.avg_pool2d(out, out.size()[2:])
			# print("Size 7:\t",out.size())
			out = out.view(out.size(0), -1)
			# print("Size 8:\t",out.size())
			out1 = self.linear(out)
			# print("Size 9:\t",out1.size())
...
def wrn28_10(num_classes=200, drop_rate= 0.5, loss_type = 'dist'):
